use crate::backend::web::middleware::mw_auth::CtxW;
use axum::extract::State;
use axum::response::{IntoResponse, Response};
use axum::routing::post;
use axum::{Json, Router};
use crate::backend::model::ModelManager;
use crate::backend::rpc::all_rpc_router_builder;
...
async fn rpc_axum_handler(
	...
	let rpc_req = match rpc_router::Request::try_from(rpc_req) {
		...
		Err(rpc_req_error) => {
			let res =
				crate::backend::web::Error::RpcRequestParsing(rpc_req_error).into_response();
			return res;
		}
	};
	...
	let res: crate::backend::web::Result<_> = res.map_err(crate::backend::web::Error::from);
